...
based on the mappings defined in section [types](#Types).

The global interpreter lock (GIL) is held for the duration of the call.

The `default` argument, if set, should be a callable object and is
called to serialize objects that are not serializable. `default` is
called with one argument, an object to serialize, and its return value
is used as the serializable representation of the object. If the
return value is not serializable, `default` is called recursively, up
to 254 times.
...
If an object is not handled, `default` should raise an exception.
Otherwise, the object is serialized as `None`, because of Python
implicit [call return
...
more serialization options. Options are integers and can be combined
using the bitwise OR operator (`|`), e.g., `ormsgpack.OPT_NON_STR_KEYS
| ormsgpack.OPT_NAIVE_UTC`.

`packb()` raises `MsgpackEncodeError` (a subclass of `TypeError`) if

- an object is not serializable
...
in MessagePack format to a Python object, based on the following mappings:
...
- timestamp extension objects are deserialized as UTC `datetime.datetime` instances, if [`OPT_DATETIME_AS_TIMESTAMP_EXT`](#OPT_DATETIME_AS_TIMESTAMP_EXT) is specified
...
the object is of an invalid type or is not valid MessagePack.
