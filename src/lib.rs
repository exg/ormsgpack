...
fn decode_opts(opts: *mut PyObject, mask: i32) -> Result<i32, ()> {
    let ob_type = ob_type!(opts);
    if is_type!(ob_type, typeref::INT_TYPE) {
        let val = ffi!(PyLong_AsLong(opts)) as i32;
        if val & !mask == 0 {
            Ok(val)
        } else {
            Err(())
        }
    } else if is_type!(ob_type, typeref::NONE_TYPE) {
        Ok(0)
    } else {
        Err(())
    }
...
pub unsafe extern "C" fn unpackb(
    ...
    if let Some(opts) = optsptr {
        match decode_opts(opts.as_ptr(), opt::UNPACKB_OPT_MASK) {
            Ok(val) => {
                optsbits = val;
            }
            Err(()) => {
                return raise_unpackb_exception("Invalid opts");
            }
        }
    }
...
pub unsafe extern "C" fn packb(
        ...
        for i in 0..tuple_size {
            ...
            if arg == typeref::DEFAULT {
                if unlikely!(default.is_some()) {
                    return raise_packb_exception("packb() got multiple values for argument: 'default'");
                }
                default = Some(NonNull::new_unchecked(*args.offset(num_args + i)));
            } else if arg == typeref::OPTION {
                if unlikely!(optsptr.is_some()) {
                    return raise_packb_exception("packb() got multiple values for argument: 'option'");
                }
                optsptr = Some(NonNull::new_unchecked(*args.offset(num_args + i)));
            } else {
    ...
    let mut optsbits: i32 = 0;
    if let Some(opts) = optsptr {
        match decode_opts(opts.as_ptr(), opt::PACKB_OPT_MASK) {
            Ok(val) => {
                optsbits = val;
            }
            Err(()) => {
                return raise_packb_exception("Invalid opts");
            }
        }
    }
