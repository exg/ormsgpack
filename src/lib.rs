...
pub unsafe extern "C" fn ormsgpack_exec(op: *mut PyObject) -> c_int {
    ...
    let result: PyResult<()> = (|| {
        let module_state = &mut *PyModule_GetState(op).cast::<ModuleState>();
        let state = module_state.set(state::State::new(py)?);

        let ext_type = state.serialize.ext.type_object.bind_borrowed(py);
        let fragment_type = state.serialize.fragment.type_object.bind_borrowed(py);
        let decode_error = state.deserialize.MsgpackDecodeError.bind_borrowed(py);
        let encode_error = state.serialize.MsgpackEncodeError.bind_borrowed(py);
...
pub unsafe extern "C" fn unpackb(
    ...
    let py = Python::assume_attached();
    let module_state = &*PyModule_GetState(module).cast::<ModuleState>();
    let state = module_state.get();
