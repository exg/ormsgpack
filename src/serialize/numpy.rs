...
impl NumpyTypes {
    ...
    fn load(py: Python<'_>) -> PyResult<Option<Self>> {
        ...
        unsafe {
            Ok(Some(Self {
                bool_: Py::from_bound(numpy.getattr("bool_")?),
                datetime64: Py::from_bound(numpy.getattr("datetime64")?),
                float16: Py::from_bound(numpy.getattr("half")?),
                float32: Py::from_bound(numpy.getattr("float32")?),
                float64: Py::from_bound(numpy.getattr("float64")?),
                int8: Py::from_bound(numpy.getattr("int8")?),
                int16: Py::from_bound(numpy.getattr("int16")?),
                int32: Py::from_bound(numpy.getattr("int32")?),
                int64: Py::from_bound(numpy.getattr("int64")?),
                uint8: Py::from_bound(numpy.getattr("uint8")?),
                uint16: Py::from_bound(numpy.getattr("uint16")?),
                uint32: Py::from_bound(numpy.getattr("uint32")?),
                uint64: Py::from_bound(numpy.getattr("uint64")?),
                ndarray: Py::from_bound(numpy.getattr("ndarray")?),
            }))
        }
    }
}
...
impl State {
    ...
    pub fn new(py: Python<'_>) -> Self {
        unsafe {
            Self {
                types: OnceLock::new(),
                array_struct_str: Py::from_bound(PyString::intern(py, "__array_struct__")),
                descr_str: Py::from_bound(PyString::intern(py, "descr")),
                dtype_str: Py::from_bound(PyString::intern(py, "dtype")),
            }
        }
    }
