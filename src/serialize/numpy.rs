...
unsafe fn get_type(module: *mut PyObject, type_name: &CStr) -> *mut PyTypeObject {
    PyObject_GetAttrString(module, type_name.as_ptr()).cast()
}
...
impl State {
    ...
    pub fn get_types(&self) -> &Option<NumpyTypes> {
        self.types.get_or_init(NumpyTypes::load)
    }
}
...
impl NumpyArray {
    ...
    pub fn try_new(
        ...
        if obj.get_type_ptr() == types.array {
    ...
    ($name:ident, $object_name:ident, $type:ty, $type_name:ident) => {
        ...
        impl $name {
            ...
            pub fn try_new(obj: PyObjectWithType, types: &NumpyTypes) -> Option<Self> {
                if obj.get_type_ptr() == types.$type_name {
...
impl<'a> NumpyDatetime64<'a> {
    ...
    pub fn try_new(
        ...
        if obj.get_type_ptr() == types.datetime64 {
            Some(Self {
                ptr: obj.as_ptr(),
                state,
                opts,
...
impl NumpyFloat16 {
    #[inline]
    pub fn try_new(obj: PyObjectWithType, types: &NumpyTypes) -> Option<Self> {
        if obj.get_type_ptr() == types.float16 {
            Some(Self { ptr: obj.as_ptr() })
        } else {
            None
        }
