        ...
        if !tp_dict.is_null()
            && unsafe {
                pyo3::ffi::PyDict_Contains(tp_dict, state.pydantic.fields_str) == 1
                    || pyo3::ffi::PyDict_Contains(tp_dict, state.pydantic.pydantic_validator_str)
                        == 1
            }
        {
            Some(Self {
...
impl Serialize for PydanticModel<'_> {
        ...
        S: Serializer,
    {
        ...
        let maybe_extra_dict =
            unsafe { pyobject_getattr(self.ptr, self.state.pydantic.pydantic_extra_str) };
...
impl PydanticModel<'_> {
        ...
        S: Serializer,
    {
        ...
        critical_section.begin(dict, extra_dict);
