...
impl State {
    ...
    pub fn new(py: Python<'_>) -> PyResult<Self> {
        unsafe {
            Ok(Self {
                field_type: Py::from_bound(py.import("dataclasses")?.getattr("_FIELD")?),
                dataclass_fields_str: Py::from_bound(PyString::intern(py, "__dataclass_fields__")),
                field_type_str: Py::from_bound(PyString::intern(py, "_field_type")),
            })
        }
    }
}
