...
pub fn deserialize<'a, 'py>(
    ...
    let py = obj.py();
    let input = BorrowedWithType::new(obj);

    if let Some(obj) = input.cast_exact::<PyBytes>() {
        let contents = pybytes_as_bytes(obj);
        deserialize_slice(py, contents, state, ext_hook, opts)
    } else if let Some(obj) = input.cast_exact::<PyMemoryView>() {
    ...
    } else if let Some(obj) = input.cast_exact::<PyByteArray>() {
    ...
    R: Read,
{
    ...
    fn deserialize_array(&mut self, len: u32) -> Result<Bound<'py, PyList>, Error> {
        ...
        let mut iter = PyIter {
            remaining: len,
            next: || self.deserialize().map_err(|e| e.into_pyerr(py, state)),
