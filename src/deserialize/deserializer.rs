...
pub fn deserialize(
    ...
    let mut deserializer = Deserializer::new(contents, context, ext_hook, opts);
    deserializer
        .deserialize()
...
impl<'de> Deserializer<'de> {
    fn new(data: &'de [u8], context: *mut Context, ext_hook: Option<NonNull<pyo3::ffi::PyObject>>, opts: Opt) -> Self {
        Deserializer {
            data: data,
            context: context,
            ext_hook: ext_hook,
    ...
    fn deserialize_map_str_key(&mut self, len: u32) -> Result<NonNull<pyo3::ffi::PyObject>, Error> {
        if unlikely!(len > 64) {
            ...
            let data = self.read_slice(len as usize)?;
            Ok(unsafe { (*self.context).key_map.get(data)? })
        }
    }
